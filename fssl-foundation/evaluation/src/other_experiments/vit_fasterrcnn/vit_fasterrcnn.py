...
class VitWithFPN(nn.Module):
    def __init__(self, vit_model, fpn_channels=256):
        ...
        self.stage2 = nn.ModuleList(
            self.vit.blocks[blocks_per_stage : 2 * blocks_per_stage]
        )
        self.stage3 = nn.ModuleList(
            self.vit.blocks[2 * blocks_per_stage : 3 * blocks_per_stage]
        )
        ...
        self.upsample = nn.ConvTranspose2d(
            fpn_channels, fpn_channels, kernel_size=2, stride=2
        )
    ...
    def forward(self, x):
        ...
        for block in self.stage4:
            x = block(x)
        features["layer4"] = self.downsample(
            self._reshape_and_project(x, self.proj4, B)
        )
    ...
    if "pos_embed" in state_dict:
        ...
        if orig_size != new_size:
            ...
            pos_tokens = (
                ...
                .reshape(-1, orig_size, orig_size, embedding_size)
                .permute(0, 3, 1, 2)
            )
            ...
            new_pos_embed = torch.cat(
                (pos_embed_checkpoint[:, :num_extra_tokens], pos_tokens), dim=1
            )
...
if __name__ == "__main__":
    ...
    # Options: "random" or "pretrained"
    BACKBONE_TYPE = "pretrained"  # Change to "random" for random weights

    DINO_CHECKPOINT_PATH = "../../ZODPretraining/src/outputs/checkpoint.pth"
    # ==============================

    if BACKBONE_TYPE == "pretrained":
        print("=== Using DINO-pretrained ViT backbone ===")
        vit = vit_small(img_size=[1024, 1024], patch_size=16)
        pretrained_vit = load_trained_vit(
            model=vit, dino_checkpoint_path=DINO_CHECKPOINT_PATH, network="student"
        )
        model = create_vit_fasterrcnn(num_classes=4, vit_model=pretrained_vit)
    else:
        print("=== Using random ViT backbone ===")
        vit = vit_small(img_size=[1024, 1024], patch_size=16)
