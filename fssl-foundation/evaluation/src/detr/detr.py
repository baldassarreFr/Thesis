...
class ZODROIFar(Dataset):
    ...
    def __init__(
        ...
        self.class_mapping = {
            ...
            "VulnerableVehicle": 1,
            "Pedestrian": 2,
        }  # for DETR, the mapping should start at 0
        self.crop = ODFarCrop()
    ...
    def __getitem__(self, idx):
        ...
        bboxes, labels = rescale_filter_cropped(
            bboxes,
            labels,
            top=924,
            left=1284,
...
def generate_detr_coco_ground_truth(
    dataset, output_path="coco_ground_truth_pedestrian.json"
):
    coco_ground_truth = {
        "images": [],
        "annotations": [],
    ...
    for image, target, coco_bboxes in dataset:
        ...
        image = {"id": int(image_id), "width": orig_width, "height": orig_height}
...
def detr_predictions_to_coco(test_dataset, model, processor, threshold=0.5):
    generate_detr_coco_ground_truth(
        test_dataset, output_path="coco_ground_truth_pedestrian.json"
    )
            ...
            for target, prediction in zip(labels, predictions):
                ...
                for score, box, label in sorted_predictions:
                    ...
                    pred_bbox = {
                        "image_id": image_id,
                        "category_id": label
                        + 1,  # Convert from DETR (0-based) to COCO (1-based) format
