...
CUDA_LAUNCH_BLOCKING=1 uv run torchrun --standalone --nproc-per-node=1 -m plain_detr.main \
    --args.output_dir ./exps/zod_dinov3 \
    --args.dataset_file zod \
    --args.zod_path /root/zod-dataset \
    ...
    --args.warmup 1000 \
    --args.use_wandb \
    --args.wandb_name zod_dinov3_vit_small_run_fixed_1 \
    "$@"
